...
% Last Modified by GUIDE v2.5 26-Sep-2012 15:28:04
...
try
    ...
    offsetpeaks = neighborPeaksAndSizes(handles);
    
    for cellNum=1:length(tracesToProcess)
        thesePeaks=offsetpeaks{cellNum};
        thesePeaks=thesePeaks-offsetFrames;
        thesePeaks=thesePeaks(thesePeaks>0);
        offsetpeaks{cellNum}=thesePeaks;
    end
...
function offsetpeaks = neighborPeaksAndSizes(handles)
