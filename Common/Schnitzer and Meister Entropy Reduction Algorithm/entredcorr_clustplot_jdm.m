...
if (nargin<2)|(isempty(entthresh))
    ...
    % Shuffled data:
    shuffmin = 10; %not actually used... %100; % Minimum number of time-bins for shuffle;
...
%     threshtol = min(5e-3, nICs*(nICs-1)/2) % Fraction of shuffled cell pairs above tolerance
    ...
    jgreaterk = jmat>kmat; % find single indicies for this for addressing later
    tshuff = [];
    
    %shuffle indicies - this is not a constant offset
    for j=1:nICsorig
        tshuff = [tshuff,randperm(nsamp)]; % Choosing shuffles this way ensures no two shuffles will be identical
    end
    tic
    
    %convergence after a minimum number of shuffles
    while ((~convergeFlag)||(jshuff<nshuff(1)))&&(jshuff<nshuff(2))
        ...
        % Compute overall change in entropy matrix 
...
l=0; %number of rounds
nICs = size(spmat,2);
nICsorig = nICs;
maxentred(1) = 1;
%original is the total entropy with each 
totent = sum(ent(sprate))*ones(nICs,1);
maxentred = zeros(nICs,1);
maxentred(end) = 1; %nice function

% Compute initial entropy matrix
sprate2 = (spmatred' * spmatred) / nsamp; %dot product of each pair
[spratej,spratek] = meshgrid(sprate); %replicates rates in each direction
entred = ent(spratej) + ent(spratek) - ... %upper and lower triangular
    (ent(spratej - sprate2) + ent(spratek - sprate2) + ent(sprate2));

entred = triu(entred, 1) - tril(ones(size(entred)),0);
%upper triangular without diagonal, lower with diagonal
%don't want to find anything on the lower diagonal

%total unmodified entropy
totent(nICs) = sum(ent(sprate));

%matricies are dynamically expanded
while (maxentred(end)>entthresh)&&(l<maxrounds)
    ...
    if maxentred(nICs+1)>entthresh
        nICs = nICs+1;
        %find the pair of max entropy reducing indicies
        ...
        %modify the time traces, rate matrix, and total entropy after
...
%removed 'zones'
entplot(constitcells, totent, entthresh, idx);
...
%doesn't depend on the specific pair/assumes all have same baseline firing?
