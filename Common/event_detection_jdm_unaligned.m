...
function [finalpeaks,offsetpeaks,peakrise,thresh] = event_detection_jdm_unaligned(tracesfile,imagesfile,params,raster_file)
...
finalpeaks_neighbors = cell(numCells,1);
offsetpeaks_neighbors = cell(numCells,1);
peakrise_neighbors = cell(numCells,1);
...
fprintf('Getting Peak Positions. \n')
for c=1:size(celltraces,1)
    ...
    fprintf('Starting neighbor subtraction. \n')
...
%                 superiorPeaks = cat(1,superiorPeaks,rankingmatrix(1,:));
    ...
    finalpeaks_neighbors{c} = testpeaks(~othersgreater);
    offsetpeaks_neighbors{c} = offsetpeaks{c}(~othersgreater);
    peakrise_neighbors{c} = peakrise{c}(~othersgreater);
    end
    
    
           finalpeaks{c} = testpeaks;
    offsetpeaks{c} = offsetpeaks{c};
    peakrise{c} = peakrise{c};
 
      if isempty(finalpeaks_neighbors{c})
        finalpeaks_neighbors{c} = zeros(1,0);
        offsetpeaks_neighbors{c} = zeros(1,0);
        peakrise_neighbors{c} = zeros(1,0);
    end
...
save(strrep(raster_file,'.mat','_neighbors.mat'),'finalpeaks_neighbors','offsetpeaks_neighbors','peakrise_neighbors');


%% draw plot of cells and neighbors
if makeThePlot
