function M = animate_markers_aligned_fullmovie_clusters(mocapstruct,frame_inds,clusterno,chunksize)
%matlab_fr = 10;
h=figure(370)
...
frameind_diff = cat(1,0, reshape(diff(frame_inds),[],1));
frameind_diff(frameind_diff<chunksize+1) = 0;
frameind_diff(frameind_diff>0) = 1;
...
for lk = reshape(frame_inds,1,[])%1:10:10000
     cla;
    inst_no =inst_no+ frameind_diff(find(frame_inds==lk));
  %  set(handles.t4,'String',num2str(lk));
  if frameind_diff(find(frame_inds==lk)) == 1
    frameshow = 0;
 % else
  end
...
%                     squeeze(mocapstruct.markers_aligned_preproc.(mocapstruct.markernames{mocapstruct.links{mm}(2)})(ind_to_plot,3))],'Color',mocapstruct.markercolor{mocapstruct.links{mm}(1)},'Linewidth',3);
    ...
    frame_last = lk;
                title(strcat('  Cluster number: ',num2str(clusterno),'  Frame: ' ,num2str(frameshow-chunksize),' inst no: ', num2str(inst_no)),'Color','w')
      frameshow = frameshow+chunksize;
