...
%
% ML_features.pose_window_sd = zeros(numel(difforders),size(dyn_score_whitened,1),num_eigenpcs);
% ML_features.pose_velocity = zeros(numel(difforders),size(dyn_score_whitened,1),num_eigenpcs);
...
%         [ML_features.pose_velocity(ll,:,mm),ML_features.pose_window_sd(ll,:,mm)] = get_vector_velocity(feature_mat_dyn_centered(:,mm),params);
%
%     end
% end
...
%    caxis([0 0.1])
%     ax(2)= subplot(2,1,2)
%     plot(0:1./300:(size(agg_features_here(:,jj),1)-1)./300,agg_features_here(:,jj))
%     linkaxes(ax,'x')
...
if (do_wavelets)
    ...
    tic
    [wavelets]= return_wavelets(cat(2,clipped_pre.SpineL(clustering_ind,:),clipped_pre.SpineF(clustering_ind,:),clipped_pre.Offset1(clustering_ind,:)),...
        1:numel(clustering_ind),opts);
...
% %
% %
%  subset = 1:100:100*2000;
%  mapped = tsne(ML_features.spectrogram_pcs_head(subset,1:15));
...
segment_pairs = {{'HeadB','HeadL'},{'HeadF','HeadB'},{'HeadB','SpineF'},{'SpineF','SpineM'} ,...
    ...
    {'SpineF','ShoulderL'},{'SpineF','ShoulderR'},...
    {'ShoulderL','ElbowL'},{'ElbowL','ArmL'},{'ShoulderR','ElbowR'},{'ElbowR','ArmR'},...
    {'HipL','KneeL'},{'KneeL','ShinL'},{'HipR','KneeR'},{'KneeR','ShinR'}};
...
%     dyadic_spectrograms = log( dyadic_spectrograms);
    ...
    if (~exist(jadyn_coeff_file,'file') || overwrite_coeff)
        [COEFFS_feat{kk}, dyadic_spectrograms_score, ~, ~,explained] = pca(squeeze(dyadic_spectrograms)');
                [COEFFS_feat_wl{kk}, dyadic_spectrograms_score_wl, ~, ~,explained_wl] = pca(squeeze(dyadic_spectrograms_wl));
    ...
    ML_features.(strcat('spectrogram_pcs_',angle_list_name{kk},'_explained')) = explained;
        ML_features.(strcat('spectrogram_pcs_wl_',angle_list_name{kk},'_explained_wl')) = explained_wl;
