...
% training set
ratnames = ["JDM25","Vicon8"] ;
condition_numbers_vec = [5;6] ;
annotation_numbers = [4;2] ;

ratnames = ["Vicon8"] ;
condition_numbers_vec = [6] ;
annotation_numbers = [2] ;

ratnames = ["JDM25"] ;
condition_numbers_vec = [5] ;
annotation_numbers = [4] ;

% testing set 
ratnames = ["JDM33"] ; 
condition_numbers_vec = [1] ; 
annotation_numbers = [5] ; 

% testing set 
ratnames = ["Vicon8"] ; 
condition_numbers_vec = [1] ; 
annotation_numbers = [3] ; 

% load all 
ratnames = ["Vicon8";"Vicon8";"JDM25";"JDM33";"JDM25";"JDM25";"JDM33";"JDM33"];
condition_numbers_vec = [6;1;5;1;1;1;1;1];
annotation_numbers = [2;3;4;5;7;8;9;10];

% model May 2 
ratnames = ["JDM25","Vicon8","Vicon8"] ;
condition_numbers_vec = [5;6;1] ;
annotation_numbers = [4;2;3] ;


mocapmasterdirectory = 'Y:\Jesse\Data\Motionanalysis_captures\';

tot_ML_features = struct([]) ;  
tot_annotation_output_goodtracking = [] ;  
tot_annotated_frames_goodtracking = [] ;
tot_fieldnames_beh = {} ;
% tot_observed_behaviors = {''} ; 
tot_subset_of_frames_annotated = [] ; 
...
for i = 1
    ...
    [fullposture_annotation_struct,fulloutput_annotation_struct,subset_of_frames_annotated,annotation_filenumber] = load_handannotation_files(annotation_number,mocapmasterdirectory);
...
%     for ll = 2:max(annotation_filenumber{:})
%             fprintf('starting a merge for mocap number %f \n',ll);
...
%     tot_fieldnames_beh = union(tot_fieldnames_beh, fieldnames_beh) ;
%     tot_observed_behaviors = union(tot_observed_behaviors, observed_behaviors(2:end)) ;   
end
...
fb = tot_fieldnames_beh ; 
ft = tot_ML_features ; 

% access copy
tot_annotated_frames_goodtracking = frames ;
tot_annotation_output_goodtracking = labels ; 
tot_fieldnames_beh = fb; 
tot_ML_features = ft; 

tot_fieldnames_beh = tot_fieldnames_beh(2:end) ; 
% equivalence classes
[predicted_labels_equivalent,new_behavioral_labels] = get_equivalence_classes(tot_annotation_output_goodtracking,tot_fieldnames_beh) ;
tot_fieldnames_beh = new_behavioral_labels ; 
tot_annotation_output_goodtracking = predicted_labels_equivalent ; 
...
for i = 1:length(bad_behs)
    ...
    bad_num = find(strcmp(tot_fieldnames_beh,current_beh)==1) - 1;
    frames = tot_annotated_frames_goodtracking;
    labels = tot_annotation_output_goodtracking;
    indices = labels ~= bad_num;
    tot_annotated_frames_goodtracking = frames(indices);
    tot_annotation_output_goodtracking = labels(indices);
end

%% RUN THE ML ALGORITHM TO FIND SIMILAR FRAMES

frames_to_annotate = 1:max(tot_subset_of_frames_annotated); %zeros(length(tot_subset_of_frames_annotated),1) + 1; 
frames_to_annotate_2 = setdiff(tot_subset_of_frames_annotated, fixed_subset,'stable');
fta3 = tot_subset_of_frames_annotated(1:278334) ; 

tic
clear candidate_frames
clear Mdl_test
[candidate_frames,score,Mdl_test,annotated_instances] = findsimilarframes_mlfeatures_multifeatures_2(tot_ML_features,tot_annotation_output_goodtracking,tot_annotated_frames_goodtracking,frames_to_annotate);
toc

% [candidate_frames,score,Mdl_test,annotated_instances] = findsimilarframes_mlfeatures_multifeatures(ML_features,annotation_output_goodtracking,annotated_frames_goodtracking,fta3);

oob_err = oobError(Mdl_test,'mode','ensemble') ;

[all_behaviors,cfmat_percent,cfmat_rawnum,tp_rate] = beh_analysis(annotated_instances,candidate_frames,tot_fieldnames_beh);
...
trained_treebagger = load('Vicon8caff_Mdl_test_annotation_ii.mat') ;
trained_Mdl_test = trained_treebagger.Mdl_test ; 
Reference_fieldnames = load('Vicon8caff_tot_fieldnames_beh_annotation_ii.mat') ;
good_fieldnames_beh = Reference_fieldnames.tot_fieldnames_beh ; 
...
% [final_fieldnames,switched_labels] = reorder_labels(good_fieldnames_beh,tot_fieldnames_beh,labels);
...
for j = 1:length(tot_fieldnames_beh)
    ...
    if sum(strcmp(current_beh,good_fieldnames_beh)) ~= 0
        ...
        %number in model
        good_beh_number = find(strcmp(good_fieldnames_beh,current_beh)==1) - 1;
        %number in own dataset
        beh_num_to_change = find(strcmp(tot_fieldnames_beh,current_beh)==1) - 1;
        if good_beh_number ~= beh_num_to_change
            indices_to_change = (original_labels_newfile == beh_num_to_change) ;
            tot_annotation_output_goodtracking(indices_to_change) = good_beh_number; 
        end
        
        new_fieldnames_beh(good_beh_number + 1) = {current_beh} ; 
    % if current behavior has not been observed (NEED TO DECIDE HOW TO CLASSIFY THIS)                
        ...
        new_beh_num = length(good_fieldnames_beh) - 1 ;
...
new_fieldnames_beh = new_fieldnames_beh' ;
emptyCells = cellfun(@isempty,new_fieldnames_beh);
new_fieldnames_beh(emptyCells) = {'null'} ; 

clear candidate_frames
[candidate_frames,annotated_instances] = test_new_rat(tot_ML_features,tot_annotation_output_goodtracking,tot_annotated_frames_goodtracking,trained_Mdl_test);

[all_behaviors,cfmat_percent,cfmat_rawnum,tp_rate] = beh_analysis(annotated_instances,candidate_frames,new_fieldnames_beh);
