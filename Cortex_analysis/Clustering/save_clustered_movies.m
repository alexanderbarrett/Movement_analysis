...
function M = save_clustered_movies(mocapstruct,ind_cell,savefolder,tag)
    ...
    for lk = 1 %numel(movie_output{jjj})
        ...
        %   h{ll}=subplot_tight(nrows,ncols, ll);
         ...
         movielength = min(numel(ind_cell{ll}),1000);
                 M =      animate_markers_aligned_fullmovie_clusters(mocapstruct,ind_cell{ll}(1:10:movielength),ll,10);
                v = VideoWriter(strcat(savefolder,filesep,tag,'_clusteredmovie_',num2str(ll)),'MPEG-4');
    open(v)
    writeVideo(v, M)
    close(v)
