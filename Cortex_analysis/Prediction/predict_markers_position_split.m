function predict_markers_position(mocapstruct,mod_1,comp_type)
...
axis_order = [1,2,3,4,6,7,13,12,11,14,15,16,8,9,18,19,10,17,20];
axis_order_fit  = axis_order;
axis_order_fit(axis_order>4) = axis_order(axis_order>4)-1;
figure(6)
imagesc(   fitlosses_norm(axis_order_fit,axis_order_fit))
caxis([0 1.2])
xlabel('Predicted Marker')
ylabel('Predictor Marker')
h=colorbar;
ylabel(h, 'Cross-Val MSE/MS Power')
set(gca,'XTick',1:numel(comparison_groups),'YTick',1:numel(comparison_groups),'XTickLabels',mocapstruct.markernames( comparison_groups(axis_order_fit)),'YTickLabels',mocapstruct.markernames( comparison_groups(axis_order_fit)));
