function [markers, markers_aligned] = align_hands_elbows_task(markers,fps)
...
marker_M_lowpass =zeros(size(markers.SpineF));
marker_L_lowpass =zeros(size(markers.SpineM));

%% shouldn't be any nans, but if there are they screw up filtfilt
markers.SpineM(isnan(markers.SpineM)) = 0;
markers.SpineL(isnan(markers.SpineL)) = 0;
...
marker_L_lowpass(:,mk) = filtfilt(f1,f2,...
            markers.SpineL(:,mk));
end

%% get the marker rotation matrix
fprintf('rotating markers')
rotangle = atan2(-(marker_M_lowpass(:,2)-marker_L_lowpass(:,2)),...
    (marker_M_lowpass(:,1)-marker_L_lowpass(:,1)));
...
cluster_mean_array =  (markers.SpineL(: ,:));
...
vec1 = marker_M_lowpass(:,[1,3])-marker_L_lowpass(:,[1,3]);
